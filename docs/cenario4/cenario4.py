...
#csv_medio = []
dataframes = []
colunas = ['Teste','Número de Testes','Algoritmo','Tempo de Execução','Número de Comparações','Número de Colisões','Gasto de Memoria']
...
for g_name in grupo.groups:
    ...
    m_colisoes = df[colunas[5]].mean()
    m_memoria = df[colunas[6]].mean()
...
for i in range(len(algo_dfs)):#algoritmos
    print('\n---',algo_names[i],'---')
    print(str(colunas[1])+'&\t'+str(colunas[3])+'&\t'+str(colunas[4])+'&\t'+str(colunas[5])+'&\t'+str(colunas[6]))
    for j in range(len(eixo_x)):#testes
        vm = list(algo_dfs[i][colunas[6]])[j]
        vm /= (1024*1024)
        
        testes = str(list(algo_dfs[i][colunas[1]])[j])
        tempo = str(list(algo_dfs[i][colunas[3]])[j])
        comp = str(list(algo_dfs[i][colunas[4]])[j])
        colisoes = str(list(algo_dfs[i][colunas[5]])[j])
        memoria = str(vm)
        print(testes+'&\t'+tempo+'&\t'+comp+'&\t'+colisoes+'&\t'+memoria)
...
plt.xlabel(u"Número de elementos")
plt.ylabel(u"Trocas")
plt.title(u"Consumo de memória (Megabytes)")
plt.legend()
